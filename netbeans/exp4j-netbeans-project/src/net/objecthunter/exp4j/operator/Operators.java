...
public abstract class Operators {
    ...
    private static final int NUM_OPERATORS = 8;

    private static final Operator[] builtinOperators = new Operator[NUM_OPERATORS];
    ...
    public static Operator getBuiltinOperator(final char symbol, final int numArguments) {
        switch(symbol) {
            case '+':
                return numArguments != 1 ? builtinOperators[INDEX_ADDITION] : 
                    builtinOperators[INDEX_UNARYPLUS];
            case '-':
                return numArguments != 1 ? builtinOperators[INDEX_SUBTRACTION] : builtinOperators[INDEX_UNARYMINUS];
            case '*':
