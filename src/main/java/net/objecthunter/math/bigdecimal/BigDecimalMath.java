...
public class BigDecimalMath
{
	...
	static public BigDecimal sqrt(final BigDecimal x, final MathContext mc)
	{
		...
		MathContext locmc = new MathContext(mc.getPrecision() + 2,
				mc.getRoundingMode());
	...
	static public BigDecimal sin(final BigDecimal x)
	{
		...
		else
		{
			...
			int prec = err2prec(3.14159, errpi);
			MathContext mc = new MathContext(2 + prec);
			BigDecimal p = pi(mc);
			mc = new MathContext(x.precision());
			if (res.compareTo(pi(new MathContext(prec))) == 0) {
				return BigDecimal.ZERO;
	...
	static public double psi(final double x)
	{
		...
		else
		{
			...
			return resul + 0.422784335098467139393487909917597568
					+ 0.5 / xmin1 - 1. / (1 - xmin1 * xmin1) - Math.PI
	...
	static public BigComplex multiplyRound(final BigComplex x,
			final BigComplex y)
	{
		BigDecimal R = subtractRound(multiplyRound(x.re, y.re),
				multiplyRound(x.im, y.im));
		BigDecimal I = addRound(multiplyRound(x.re, y.im),
