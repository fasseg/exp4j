...
public class BigIntegerMath {
	...
	static public BigInteger binomial(final int n, final int k) {
		...
		BigInteger n2 = bin;
		for (BigInteger i = new BigInteger("" + (k - 1)); i
				.compareTo(BigInteger.ONE) >= 0; i = i.subtract(BigInteger.ONE))
	...
	static public BigInteger binomial(final BigInteger n, final BigInteger k) {
		 ...
		 * = i.subtract(BigInteger.ONE) ) bin = bin.multiply(n2.subtract(i)) ; i= new BigInteger(""+k) ; for( ;
	 ...
	 * @author Richard J. Mathar
	 */
	static public BigInteger[][] minor(final BigInteger[][] A, final int r,
			final int c) throws ArithmeticException {
		/* original row count */
		final int rL = A.length;
		if (rL == 0)
			throw new ArithmeticException("zero row count in matrix");
		if (r < 0 || r >= rL)
			throw new ArithmeticException("row number " + r
					+ " out of range 0.." + (rL - 1));
		...
		if (c < 0 || c >= cL)
			throw new ArithmeticException("column number " + c
					+ " out of range 0.." + (cL - 1));
	 ...
	 * @param v
	 *            The column vector to be inserted. With the current implementation, it must be at least as long as the
	...
	static private BigInteger[][] colSubs(final BigInteger[][] A, final int c,
		...
		for (int row = 0; row < rL; row++) {
			for (int col = 0; col < cL; col++) {
				/*
				 * currently, v may just be longer than the row count, and surplus elements will be ignored. Shorter v
	 ...
	 * @author Richard J. Mathar
	 */
	static public BigInteger det(final BigInteger[][] A)
		...
		final int cL = A[0].length;
		if (cL != rL)
			throw new ArithmeticException("Non-square matrix dim " + rL
					+ " by " + cL);
	 ...
	 * @author Richard J. Mathar
	 */
	static public Rational[] solve(final BigInteger[][] A,
			final BigInteger[] rhs) throws ArithmeticException {
		...
		if (cL != rL)
			throw new ArithmeticException("Non-square matrix dim " + rL
					+ " by " + cL);
		if (rhs.length != rL)
			throw new ArithmeticException("Right hand side dim " + rhs.length
					+ " unequal matrix dim " + rL);
		...
		for (int c = 0; c < cL - 1; c++) {
			...
			if (A[c][c].compareTo(BigInteger.ZERO) == 0) {
				...
				if (!swpd)
					throw new ArithmeticException(
							"Zero determinant of main matrix");
			}
			...
			for (int r = c + 1; r < rL; r++) {
				...
				Rational tmp = x[r].multiply(A[c][c]).subtract(
						x[c].multiply(A[r][c]));
				x[r] = tmp;
			}
		}
	 ...
	 * @author Richard J. Mathar
	 */
	static public BigInteger valueOf(final Vector<BigInteger> c,
			final BigInteger x) {
		if (c.size() == 0)
	 ...
	 * @since 2009-08-06
	 * @author Richard J. Mathar
	 * @see <a href="http://dx.doi.org/10.1080/01630568908816313">P. L. Butzer et al, Num. Funct. Anal. Opt. 10
	 *      (5)(1989) 419-488</a>
	 */
