...
public class Ifactor implements Cloneable, Comparable<Ifactor> {
	...
	protected Ifactor multGcdLcm(final Ifactor oth, int type) {
		...
		if (primeexp.size() != 0 && oth.primeexp.size() != 0) {
			...
			if (primeexp.firstElement().intValue() == 1 && type == 0)
				 ...
				 */
				while (idxOth < oth.primeexp.size()
						|| idxThis < primeexp.size()) {
					if (idxOth >= oth.primeexp.size()) {
						...
						if (type == 0 || type == 2) {
							...
							prod.primeexp.add(oth.primeexp
									.elementAt(idxOth + 1));
						}
						idxOth += 2;
					} else {
						Integer p;
						int ex;
						switch (primeexp.elementAt(idxThis).compareTo(
								oth.primeexp.elementAt(idxOth))) {
						case 0:
	...
	public Ifactor pow(final int exponent) throws ArithmeticException {
		 ...
		 */
		if (exponent < 0)
			throw new ArithmeticException("Cannot raise " + toString()
					+ " to negative " + exponent);
		else if (exponent == 0)
	...
	public Rational root(final int r) throws ArithmeticException {
		if (r == 0)
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				...
				int ex = primeexp.elementAt(i + 1).intValue();
				if (ex % r != 0)
					throw new ArithmeticException("Cannot pull " + r
							+ "th root of " + toString());

				pows.multiply(new BigInteger("" + primeexp.elementAt(i)).pow(ex
						/ r));
			}
	...
	public Vector<BigInteger> divisors() {
		 ...
		 * pz, and the the divisors that contain any power of pz between 1 and up to ez multiplied by 1 or by a product
		 * that contains the factors p1..py.
		 */
		...
		if (n.compareTo(BigInteger.ONE) > 0) {
			...
			final BigInteger pz = new BigInteger(primeexp.elementAt(
					primeexp.size() - 2).toString());
		...
		if (n.compareTo(BigInteger.ONE) == 0)
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				...
				if (k == 0)
					...
					Integer p = primeexp.elementAt(i);
					BigInteger num = (new BigInteger(p.toString())).pow(
							k * (ex + 1)).subtract(BigInteger.ONE);
	...
	public BigInteger core() {
		...
		for (int i = 0; i < primeexp.size(); i += 2)
			if (primeexp.elementAt(i + 1).intValue() % 2 != 0)
				resul = resul.multiply(new BigInteger(primeexp.elementAt(i)
						.toString()));
		return resul;
	} /* Ifactor.core */
