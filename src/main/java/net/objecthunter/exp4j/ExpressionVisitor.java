...
public class ExpressionVisitor extends Exp4jGrammarBaseVisitor<Double> {
    ...
    public Double visitTerm(final Exp4jGrammarParser.TermContext term) {
        if (term.NUMBER() != null) {
            return this.parseNumber(term.NUMBER());
    ...
    private Double evaluateVariable(Exp4jGrammarParser.TermContext term) {
        ...
        if (val == null) {
            throw new IllegalArgumentException(String.format("Unset variable at pos %d: %s", term.getStart().getStartIndex(), term.getText()));
        }
        return val;
    }
    ...
    private Double evaluateFunction(final Exp4jGrammarParser.TermContext term) {
        ...
        if (func == null) {
            throw new IllegalArgumentException(String.format("Unknown function at pos %d: %s", term.getStart().getStartIndex(), term.getText()));
        }
    ...
    private double parseNumber(final TerminalNode number) {
